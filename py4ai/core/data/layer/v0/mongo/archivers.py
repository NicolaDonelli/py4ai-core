...
from py4ai.core.data.layer.v0 import Archiver
from py4ai.core.data.layer.v0.mongo.dao import MongoDAO
...
class MongoArchiver(Archiver[T]):
    ...
    def _insert(self, obj: T) -> UpdateResult:
        ...
        :return: an instance of :class:`pymongo.results.UpdateResult` with update operation's results
        """
        keys = self.dao.computeKey(obj)

        return self.collection.update_one(
            keys if isinstance(keys, dict) else {"_id": ObjectId(str(keys))},
    ...
    def first(self) -> T:
        ...
        if json is None or not isinstance(json, dict):
            raise KeyError("No element present in the collection")
        return self.dao.parse(json)
